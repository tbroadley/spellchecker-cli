...
describe('buildPersonalDictionary', () => {
  ...
  it('reads a plaintext dictionary', async () => {
    (
      await buildPersonalDictionary(['test/fixtures/dictionaries/one.txt'])
    ).should.deep.equal([/^Thisisnotaword$/, /^preprocessed$/]);
  });

  it('reads a plaintext dictionary that uses regex notation', async () => {
    (
      await buildPersonalDictionary(['test/fixtures/dictionaries/regex.txt'])
    ).should.deep.equal([/^Thisisnot.*$/, /^(pre)?processed$/]);
  });

  it('reads a JavaScript dictionary', async () => {
    (
      await buildPersonalDictionary(['test/fixtures/dictionaries/regex.cjs'])
    ).should.deep.equal([
      /^Thisisnotaword$/,
      /^(pre)?processed$/,
      /^canadaa$/i,
