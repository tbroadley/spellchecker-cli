...
parallel(
  ...
  function testSpellcheckerCLI(this: {
    ...
    it('exits with an error when run on a file with a spelling mistake', async () => {
      ...
      code!.should.equal(1);
      stdout.should.include('Unexpected unknown word `Thisisnotaword`');
    });
    ...
    it('exits with an error when run with a dictionary that does not contain the words in the given file', async () => {
      ...
      ['Colour', 'honour', 'behaviour'].forEach(word => {
        stdout.should.include(`Unexpected unknown word \`${word}\``);
      });
    });
    ...
    it('applies retext-indefinite-article when it is specified', async () => {
      ...
      code!.should.equal(1);
      stdout.should.include(
        'Unexpected article `a` before `8-year`, expected `an`',
      );
      stdout.should.include(
        'Unexpected article `a` before `hour`, expected `an`',
      );
      stdout.should.include(
        'Unexpected article `an` before `European`, expected `a`',
      );
    });
    ...
    it('applies retext-repeated-words when it is specified', async () => {
      ...
      code!.should.equal(1);
      stdout.should.include('Unexpected repeated `it`, remove one occurrence');
      stdout.should.include('Unexpected repeated `to`, remove one occurrence');
      stdout.should.include('Unexpected repeated `the`, remove one occurrence');
    });
    ...
    it('checks only the given keys in the frontmatter (1)', async () => {
      ...
      code!.should.equal(1);
      stdout.should.include('Unexpected unknown word `tbroadley`');
      stdout.should.not.include('Unexpected unknown word `documnet`');
    });
    ...
    it('checks only the given keys in the frontmatter (2)', async () => {
      ...
      code!.should.equal(1);
      stdout.should.include('Unexpected unknown word `documnet`');
      stdout.should.not.include('Unexpected unknown word `tbroadley`');
    });
    ...
    it('treats dictionary entries as if they were wrapped in ^ and $', async () => {
      ...
      code!.should.equal(1);
      stdout.should.include('Unexpected unknown word `Thisisnotaword`');
      stdout.should.include('Unexpected unknown word `preprocessed`');
    });
    ...
    it('enables suggestions by default', async () => {
      ...
      code!.should.equal(1);
      stdout.should.include(
        'Unexpected unknown word `preprocessed`, expected for example `reprocessed`',
      );
    });
    ...
    it('catch spelling mistakes inside incomplete exclude comment blocks', async () => {
      ...
      code!.should.equal(1);
      stdout.should.include('Unexpected unknown word `iinside`');
    });
...
describe('Spellchecker CLI (not parallel)', () => {
  ...
  it('exits with an error when run on a file that is included in the .gitignore and the --no-gitignore flag is passed', async () => {
    try {
      ...
      code!.should.equal(1);
      stdout.should.include('Unexpected unknown word `contians`');
    } finally {
